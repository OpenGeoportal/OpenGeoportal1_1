...
abstract class AbstractLayerDownloader implements LayerDownloader {
	...
	abstract public void downloadLayers(List<RequestedLayer> layerList) throws Exception;
	 ...
	 */
	File createNewFileObject(RequestedLayer layerObject){
		...
		String fileName = this.filterFileName(layerObject.name);
		...
		File newFile = new File(layerObject.targetDirectory, fileName + fileExtension);
		int i = 1;
		while (newFile.exists()){
			newFile = new File(layerObject.targetDirectory, fileName + i + fileExtension);
			i++;
		}
		return newFile;
	}
