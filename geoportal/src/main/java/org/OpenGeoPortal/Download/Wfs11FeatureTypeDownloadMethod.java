...
public class Wfs11FeatureTypeDownloadMethod implements UnpackagedDownloadMethod {
	...
	private BoundingBox getClipBounds() {
	 ...
	 private Map<String, String> getWfsDescribeLayerInfo()
	 	throws Exception
	 {
		// TODO should be xml
		...
		URLConnection responseConnection = this.httpRequester.sendRequest(this.currentLayer.getWfsUrl(), describeFeatureRequest, "POST");
		...
		public InputStream getResponseStream(RequestedLayer currentLayer) throws IOException {
			...
			try {
				responseConnection = this.httpRequester.sendRequest(this.currentLayer.getWfsUrl(), this.createDownloadRequest(), "POST");
			} catch (Exception e) {
