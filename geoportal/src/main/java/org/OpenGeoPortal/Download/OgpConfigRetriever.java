...
public class OgpConfigRetriever extends ConfigRetriever implements
	...
	public String getArbitrary(String configKey) throws Exception {
		/*this only works for strings in the top level.  should eventually be more generic */
		this.readConfigFile();
		return this.configContents.path(configKey).getTextValue();
	}

	@Override
	public String getWmsProxy(String institution, String accessLevel) throws IOException {
		this.readConfigFile();
		...
		if (institutionObject.has("proxy")){
			...
			//ArrayNode accessArray = (ArrayNode) proxyObject.path("accessLevel");
			if (accessArray.contains(accessLevel)||accessArray.contains(accessLevel.toLowerCase())){
