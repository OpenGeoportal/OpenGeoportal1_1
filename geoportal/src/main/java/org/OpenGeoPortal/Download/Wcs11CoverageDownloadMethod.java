...
/**
 * currently just used for testing.  Intended to implement download via WCS 1.1
 * @author chris
 *
 */
public class Wcs11CoverageDownloadMethod implements UnpackagedDownloadMethod {
	...
	public String createDownloadRequest() throws Exception {
		...
		bounds = new BoundingBox(-11.0,8.0,-5.0,12.0);
		String layerName = this.currentLayer.getLayerNameNS();
		layerName = "sde:GISPORTAL.GISOWNER01.LANDINFO_K01C";
	...
	private Map<String, String> getWcsDescribeLayerInfo()
	 	throws Exception
	 {
	  ...
	  <wcs:SupportedFormat>image/tiff</wcs:SupportedFormat><wcs:SupportedFormat>image/tiff;subtype="geotiff"</wcs:SupportedFormat></wcs:CoverageDescription>
		...
		URLConnection responseConnection = this.httpRequester.sendRequest(this.currentLayer.getWcsUrl(), describeCoverageRequest, "GET");
		...
		public InputStream getResponseStream(RequestedLayer currentLayer) throws IOException {
			...
			try {
				responseConnection = this.httpRequester.sendRequest(this.currentLayer.getWcsUrl(), this.createDownloadRequest(), "POST");
